...
//显示一行字符，当该行原来为空白时可用，否则使用上面LCD_ShowString函数，本函数为简略版本：规定区域大小宽16长544，规定字体大小16，规定起始位置30
void LCD_Sentence(u16 y,u8 *p)
{
	u16 CONST_Width=544;
	u16 CONST_Height=16;
	u16 CONST_Size=16;
	u16 CONST_X=30;
	u16 CONST_ADD_Y=20;//第Y行乘以这个值就是调用函数的真实值
	LCD_ShowString(CONST_X , 100+y*CONST_ADD_Y , CONST_Width , CONST_Height , CONST_Size , p);
